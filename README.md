...
>
> This does not apply to process directives, where often you're providing shell systax. Example:
> ```groovy
> output:
> tuple val(meta), path("*.txt"), emit: txt
> ```
> Here you use shell syntax globbing. 
...
> ```groovy
> // Option 1: Standard print output
> println "Output: $process.out"
> // Option 2: JSON output
> def json = JsonOutput.prettyPrint(JsonOutput.toJson(process.out))
> println "json: $json"
> // Option 3: Custom helper class (RECOMMENDED)
> 
> DebugUtils.printTree(process.out)
> ```
> For the helper class to work, you need to have a `DebugUtils.groovy` file inside of `tests/lib/`. Also inside `nf-test.config`, you need to have the following line:
> ```groovy
> libDir "tests/lib"
> ```
> The helper class I wrote is currently like this:
> ```groovy
> class DebugUtils {
> 
>     // ANSI color codes
